...
# Calculate the range from the TES to the start of Exon2

# NEED TO DOUBLE CHECK THIS FUNCTION IS CALCULATING THE CORRECT RANGE (I INITIALLY WROTE IT FOR TSS -> EXON2START INSTEAD
# OF EXON2START -> TES)

# WORKING FOR + AND - STRANDS

calculate_exon2start_TES_range <- function(bed) {
  ...
  # Add metadata (optional)
  if (length(mcols(bed)) > 0) {
    mcols(tes_exon2start_ranges) <- mcols(bed)
  }

  return(tes_exon2start_ranges)
}
